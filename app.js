...
function receptionStickerPrintHtml(stickers,title){
  ...
  var css='@page{size:A4;margin:8mm}*{box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;margin:0;color:#111827;background:#fff}.no-print{padding:10px 0;margin:0 0 10px 0;text-align:center}.no-print button{border:0;border-radius:10px;background:#061b46;color:#fff;font-weight:700;padding:10px 16px;cursor:pointer}.sheet{display:grid;grid-template-columns:repeat(2,1fr);gap:8mm}.label{min-height:52mm;border:1.5px solid #111827;border-radius:8px;padding:8px;break-inside:avoid;page-break-inside:avoid;display:flex;flex-direction:column;gap:5px}.head{display:flex;justify-content:space-between;gap:8px;border-bottom:1px solid #111827;padding-bottom:5px}.brand{font-size:11px;font-weight:900;color:#061b46;text-transform:uppercase}.oc{font-size:15px;font-weight:900}.state{border:1px solid #16a34a;color:#166534;border-radius:999px;padding:2px 8px;font-size:11px;font-weight:900;align-self:flex-start}.mat{font-size:14px;font-weight:900;line-height:1.2}.grid{display:grid;grid-template-columns:1fr 1fr;gap:4px 8px;font-size:11px}.grid span{color:#475569;font-size:9px;text-transform:uppercase;letter-spacing:.03em}.grid b{display:block;font-size:11px;color:#111827}.foot{margin-top:auto;border-top:1px dashed #9ca3af;padding-top:4px;font-size:9px;color:#475569;display:flex;justify-content:space-between;gap:6px}.code{font-family:monospace;font-size:10px;color:#111827}@media print{.no-print{display:none}.sheet{gap:6mm}.label{border-color:#000}}';
  var rows=stickers.map(function(st){
    var po=receptionStickerPurchaseOrder(st)||"Sin OC";
    return '<article class="label"><div class="head"><div><div class="brand">Electroingeniería · Sticker de separación</div><div class="oc">OC: '+esc(po)+'</div></div><div class="state">'+esc(st.status||'Conforme')+'</div></div><div class="mat">'+esc(st.materialName||st.material||'Material sin nombre')+'</div><div class="grid"><div><span>Referencia</span><b>'+esc(st.reference||st.referencia||'')+'</b></div><div><span>Cantidad</span><b>'+esc(st.quantity||st.cantidad||'')+' '+esc(st.unit||st.unidad||'')+'</b></div><div><span>Ubicación</span><b>'+esc(st.location||st.ubicacion||'')+'</b></div><div><span>Medidas</span><b>'+esc(st.measures||st.medidas||'')+'</b></div><div><span>Fecha ingreso</span><b>'+esc(st.entryDate||'')+'</b></div><div><span>Responsable ingreso</span><b>'+esc(st.responsibleName||st.responsableIngreso||'')+'</b></div><div><span>Proveedor</span><b>'+esc(st.supplier||'')+'</b></div><div><span>Documento</span><b>'+esc(st.documentNumber||'')+'</b></div></div><div class="foot"><span>Generado: '+esc(new Date().toLocaleString("es-CO"))+'</span><span class="code">'+esc(st.id||'')+'</span></div></article>';
  }).join('');
