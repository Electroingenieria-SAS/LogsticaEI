...
var EI_CANONICAL_APP_VERSION = "v205-fix-x-modal-qty-csv-recepcion";
try{
...
function ensureV205UiFixes(){
  if(document.getElementById("ei-v205-fix-x-modal-qty-csv-recepcion"))return;
  var st=document.createElement("style");
  st.id="ei-v205-fix-x-modal-qty-csv-recepcion";
  st.textContent='\
\
\
\
...
function forceCajaPvcPveToCarteraNow(){
  ...
  (state.cases||[]).forEach(function(c){
    if(migrateCajaToCarteraIfNeeded(c,"Normalización masiva V205: PVC/PVE debe estar en Cartera.")){
      count++;
...
function autoNormalizeLoadedPvcPveFromCajaToCartera(reason){
  ...
  (state.cases||[]).forEach(function(c){
    ...
    if(migrateCajaToCarteraIfNeeded(c,reason||"Normalización automática V205: PVC/PVE no puede permanecer en Caja.")){
      count++;
      ...
      jobs.push(db.collection("cases").doc(c.id).set(c,{merge:true}).then(function(){
        return createEvent({type:"AUTO_CARTERA_REDIRECT_V205",caseId:c.id,process:"cartera",detail:"PVC/PVE detectado en Caja y redirigido automáticamente a Cartera.",targetRole:"cartera",visibleRoles:financialVisibleRolesForOrder(c)}).catch(function(){return null;});
...
function ensureMobileFreshVersion(){
  try{
    var version="v205-fix-x-modal-qty-csv-recepcion";
...
function loadData(){
  ...
  return Promise.all([
    ...
    state.dataLoading=false;
    var normalizePromise=autoNormalizeLoadedPvcPveFromCajaToCartera("Normalización al cargar datos V205: PVC/PVE debe ir a Cartera.");
    ...
    return normalizePromise.then(function(moved){
      if(moved>0)console.info("[V205] PVC/PVE movidos automáticamente de Caja a Cartera:",moved);
    });
  }).catch(function(e){
    state.dataLoading=false;
    throw e;
...
function autoMigrateLegacyProcesses(){
  ...
  state.cases.forEach(function(c){
    ...
    if((currentUserIsAdminOrSuper() || normalizeRole(state.user&&state.user.role)==="gerencia") && forcePveCaseToPurchasesInMemory(c,"Migración automática V205: PVE existente debe pasar por Compras antes de Recepción"))changed=true;
...
function forceExistingPveToPurchasesNow(){
  ...
  db.collection("cases").get().then(function(snap){
    ...
    list.forEach(function(c){
      ...
      if(forcePveCaseToPurchasesInMemory(c,"Migración manual V205: PVE existente enviado a Compras antes de Recepción")){
...
function loadWarningsHtml(){
  ...
  return '<div class="alert warning"><strong>Algunos módulos no cargaron por reglas/permisos pendientes.</strong><br>'+esc(state.loadWarnings.slice(0,3).join(" | "))+'<br><small>La app no se detiene; publique las reglas V205 y recargue la PWA.</small></div>';
}
...
function ensureLowResolutionResponsiveCss(){
  if(document.getElementById("ei-v205-fix-x-modal-qty-csv-recepcion")){applyLowResolutionMode();return;}
  var st=document.createElement("style");
  st.id="ei-v205-fix-x-modal-qty-csv-recepcion";
  st.textContent='\
...
function ensureV205ViewportFinalFix(){
  try{
...
@media(max-width:790px){
  ...
  }catch(e){console.warn("No se pudo aplicar V205 viewport fix",e);}
}
...
function temporaryProfileFromAuth(fbUser,reason){
  ...
  state.route=defaultRoute(state.user.role);
  state.loadWarnings=state.loadWarnings||[];
  state.loadWarnings.push("Perfil temporal: Firebase no permitió leer users/"+((fbUser&&fbUser.uid)||"")+". La app abrió para no parar el proceso. Publique reglas V205 y cree/verifique el perfil operativo.");
  sessionStorage.setItem(storageKey+"_session",JSON.stringify(state.user));
...
function createCase(fd){
  ...
  ensurePvePurchasingMetadata(c,"Creación V205: pedido PVE debe pasar por Compras antes de Recepción");
  var finish=function(){
...
function renderDetail(id){
  ...
  if(migrateCajaToCarteraIfNeeded(c,"Corrección automática V205: PVC/PVE debe ir a Cartera, no Caja."))correctedOnOpen=true;
  if(caseLooksBlockedInSales(c)){
    if(releaseAnyCaseIfResolvedByReqOrReport(c,"Corrección automática V205: requerimiento o novedad/reporte ya respondido/cerrado.","Corrección automática V205"))correctedOnOpen=true;
  }
  if(migrateLegacyCaseInMemory(c,"Corrección automática al abrir detalle"))correctedOnOpen=true;
  if((currentUserIsAdminOrSuper() || normalizeRole(state.user&&state.user.role)==="gerencia") && forcePveCaseToPurchasesInMemory(c,"Corrección automática V205 al abrir detalle: PVE debe pasar por Compras"))correctedOnOpen=true;
  ...
  if(!c.closedAt){
    ...
    if(c.status==="en_proceso"&&c.currentProcess==="recepcion_pedidos"&&canOperate)actions+='<button class="btn btn-primary" data-action="receptionPdf" data-id="'+c.id+'">Cargar PDF o CSV recepción · V205</button>';
...
function mergePdfItemsIntoCase(c, parsed){
  ...
  // V205: la clave de una línea de recepción incluye ubicación/bodega/extensiones.
...
function repairReceptionLinesFromPdfExtraction(c){
  ...
  if(added>0 || after!==before){
    ...
    c.documentFlow.receptionLinesRepairDetail="Corrección automática V205: se restauraron líneas iguales con ubicaciones distintas para que todos los carretos completos pasen a alistamiento.";
...
/* ============================================================
   V205 - Recepción de pedidos: dos botones separados PDF / CSV
   Alcance único:
...
function eiV205CsvRows(text){
  ...
  for(var r=start+1;r<lines.length;r++){
    var cols=eiV205SplitCsvLine(lines[r],delim);
    ...
    var it={
      id:'CSV-V205-'+String(rows.length+1).padStart(4,'0'),
      referencia:String(ref).replace(/\D/g,''),
      descripcion:String(desc||'').trim(),
      cantidad:eiV205NormalizeCsvQty(qty),
      ...
      generatedBy:'CSV_RECEPCION_PEDIDOS_V205',
      origen:'CSV_RECEPCION_PEDIDOS_V205',
      detectionReason:'Fila cargada desde CSV V205 en Recepción de pedidos.'
    };
...
function eiV205ReadTextFile(file){
  return new Promise(function(resolve,reject){
    var reader=new FileReader();
...
function eiV205ParsedFromCsv(text,c,fileName){
  var items=eiV205CsvRows(text);
...
function eiV205ApplyCsvItemsToCase(c,parsed){
  ...
  c.orderItems=c.orderItems.filter(function(it){
    ...
    return origin.indexOf('CSV_RECEPCION_PEDIDOS_V205')<0;
  });
...
/* V205 - Helper seguro para abrir/cerrar drawer de Recepción de pedidos */
function eiV205CloseReceptionDrawer(){
  try{
    var d=qs("#drawer");
    if(d){
...
function eiV205OpenDrawerHtml(html){
  var d=qs("#drawer");
  ...
  d.onclick=function(ev){
    ...
    if(t===d || (t && t.closest && t.closest('[data-action="closeDrawer"],[data-action="closeReceptionDrawer"]'))){
      ev.preventDefault();
      ev.stopPropagation();
      eiV205CloseReceptionDrawer();
    }
  };
  try{
    document.onkeydown=function(ev){
      if(ev && ev.key==="Escape" && d.classList.contains("open"))eiV205CloseReceptionDrawer();
    };
  }catch(e){}
}
function eiV205NormalizeCsvQty(v){
  var s=String(v||'').trim();
  if(!s)return '';
  s=s.replace(/\s+/g,'');
...
function openReceptionPdf(id){
  ...
  eiV205OpenDrawerHtml('<div class="modal modal-wide ei205-reception-modal" style="width:min(1180px,96vw);max-width:96vw;max-height:92vh;overflow:auto;"><div class="modal-head"><div><h2>Recepción de pedidos · cargar PDF o CSV · V205</h2><p>Solo afecta Recepción de pedidos. No modifica Corte, stickers ni Recepción de mercancía.</p></div><button class="icon-btn" type="button" data-action="closeReceptionDrawer" onclick="eiV205CloseReceptionDrawer()">×</button></div>'+
    '<div class="notice success"><strong>V205:</strong> escoja una carga. Use PDF si el lector funciona; use CSV cuando el PDF complejo no lea todas las líneas físicas.</div>'+
    ...
    '<button type="button" class="btn" data-action="closeReceptionDrawer" onclick="eiV205CloseReceptionDrawer()">Cancelar</button>'+
  ...
  if(csvInput){
    csvInput.onchange=function(){
      ...
      eiV205ReadTextFile(f).then(function(text){
        parsed=eiV205ParsedFromCsv(text,c,f.name);
        var auto=(parsed.items||[]).filter(function(x){return recalcReceptionItemFlags(Object.assign({},x)).requiereCorte;}).length;
  ...
  qs("#receptionPdfForm").onsubmit=function(ev){
    ...
    var changed=applyParsedPedidoToCase(c,parsed);
    c.orderItems=c.orderItems||[];
    var added=(parsed.extractionMode==="CSV_RECEPCION_PEDIDOS_V205")?eiV205ApplyCsvItemsToCase(c,parsed):mergePdfItemsIntoCase(c,parsed);
...
function ensureCutCaseFlagsV205(c){
  try{
    if(!c)return;
    c.hasCuts=true;
...
function renderCutsQueue(){
  ...
  if(isMobileRuntime()||isCutRuntimeUser()){
    ...
    layout(
      ...
      '<section class="ei191-cut-kpis"><article><span>Pendientes</span><strong>'+rows.length+'</strong></article><article><span>Grupos</span><strong>'+groupKeys.length+'</strong></article><article><span>Versión</span><strong>V205</strong></article></section>'+
...
function forceReleaseOneSalesBlock(id){
  ...
  if(!releaseCaseFromSalesBlock(c,"Liberación manual V205 por rol autorizado.","Liberación manual V205")){alert("No fue posible liberar este pedido.");return;}
  persistCase(c,{type:"SALES_BLOCK_MANUAL_RELEASED",detail:"Liberación manual V205: pedido devuelto a "+processTitle(c.currentProcess),targetRole:c.assignedRole,visibleRoles:["admin","super_admin","super_administrador","gerencia","jefe_logistica",c.assignedRole,"ventas"]}).then(loadData).then(function(){renderDetail(id);alert("Pedido liberado de Ventas y devuelto a "+processTitle(c.currentProcess)+".");}).catch(function(e){showError((e&&e.message)||e||"No se pudo liberar el pedido.");});
}
...
function forceReleaseResolvedSalesBlocksNow(){
  ...
  (state.cases||[]).forEach(function(c){
    ...
    if(releaseCaseFromSalesBlock(c,"Reparación V205: requerimiento o novedad/reporte respondido/cerrado. Incluye liberación aunque exista requerimiento y reporte al mismo tiempo.","Reparación V205")){
      changed.push(c);
      promises.push(persistCase(c,{type:"SALES_BLOCK_FORCE_RELEASED_V205",detail:"Reparación V205: pedido liberado de Ventas y devuelto a "+processTitle(c.currentProcess),targetRole:c.assignedRole,visibleRoles:["admin","super_admin","super_administrador","gerencia","jefe_logistica",c.assignedRole,"ventas"]}));
    }
  });
...
function ensureV205CutUiFixes(){
  if(document.getElementById('ei-v205-fix-x-modal-qty-csv-recepcion'))return;
  var st=document.createElement('style');st.id='ei-v205-fix-x-modal-qty-csv-recepcion';
  ...
  (state.reports||[]).forEach(function(r){
    ...
    if(cleaned.changed || wrongMerge){
      ...
      var payload={
        ...
        lastUpdateType:"CORRECCION_FORZADA_TRAZABILIDAD_V205",
        updatedAt:stamp
      };
      if(wrongMerge){
...
function downloadKpiExcel(){
  ...
  var head='<html><head><meta charset="utf-8"><style>body{font-family:Century Gothic,Arial}h1{color:#061B46}.kpi{font-size:18px;font-weight:bold;color:#061B46}table{border-collapse:collapse;width:100%;margin-bottom:18px}th{background:#061B46;color:white}td,th{border:1px solid #cbd5e1;padding:8px;vertical-align:top}.note{background:#f8fafc;border:1px solid #cbd5e1;padding:10px}.warn{background:#fff7ed}.total-hours{font-weight:bold;background:#eaf2ff}</style></head><body>'+
    ...
    '<div class="note"><strong>V205:</strong> exportación por lotes para evitar que el navegador se quede sin responder. La primera tabla sigue siendo la base del informe: DEMORA EXACTA POR PEDIDO.</div>';
  parts.push(head);
...
function renderIndicators(){
  ...
  layout(header("VSM ERP · Normal / PVE","Panel liviano con VSM normal y VSM PVE separados, más normalización de horas por pedido/día.",'<a class="btn btn-primary" href="'+link+'" target="_blank" rel="opener">Abrir VSM optimizado</a>'+(canExportSiesa?'<button class="btn btn-gold" data-action="exportSiesaCuts">Exportar plano SIESA cortes</button>':''))+
    ...
    '<section class="grid grid-3" style="margin-top:16px">'+
      ...
      '<article class="card kpi"><span>Exportación</span><strong style="font-size:1.25rem">Excel V205</strong><small>Tablas separadas para informe completo.</small></article>'+ 
    '</section>'+ 
...
function assignToProcess(c,next,detail,assignmentUsers){
  if(c && next==="recepcion_pedidos" && pveShouldBeForcedToPurchases(c)){
    ensurePvePurchasingMetadata(c,"Bloqueo V205: ningún PVE sin liberar por Compras puede entrar directo a Recepción");
    next="compras";
...
function renderCutDiagnostics(){
  ...
  layout(header("Diagnóstico Corte","Verificación rápida de perfil, permisos y carga para el usuario de Corte.",'<button class="btn btn-gold" data-action="forceRefreshCases">Probar carga de pedidos</button>')+
    ...
    '<section class="notice"><strong>Uso:</strong> si aquí el rol no sale como auxiliar_corte o no cargan casos, debe publicarse firestore.rules V205 y verificar el documento users del UID real.</section>');
}
function render(){
...
/* V205: lectura reforzada de PDF SIESA con cantidades 20.000,00 y filas multilinea; no elimina lector V92. */
function eiV205QtyRegex(){return /^\d{1,3}(?:[.,]\d{3})*(?:[.,]\d+)?$|^\d+(?:[.,]\d+)?$/;}
function eiV205CleanDescTail(s){return eiV92CleanText(String(s||'').replace(/\$\s*[\d.,]+.*$/g,'').replace(/\b(?:VALOR|UNIT|PARCIAL|SUBTOTAL|IVA|TOTAL)\b.*$/ig,''));}
function eiV205ParseSiesaRowsFromRawText(raw){
  ...
  while((m=rx.exec(text))){
    ...
    desc=eiV205CleanDescTail(desc).replace(/\s+/g,' ').trim();
    desc=desc.replace(/\bPARQUE\s+INDUSTRIAL\b/ig,' ').replace(/\s+/g,' ').trim();
    if(eiV92ReferenceOk(reference) && desc && cantidad && ubic){rows.push({referencia:reference,descripcion:desc,cantidad:cantidad,unidad:unit,ubicacion:ubic,pagina:1,mode:'V205_RAW'});}
  }
  ...
  if(!rows.length){
    ...
    var locs=[], qtys=[], units=[];
    lines.forEach(function(l){if(/^[A-Z]\d{5,6}$/.test(l))locs.push(l); if(eiV205QtyRegex().test(l))qtys.push(l); if(new RegExp('^'+unitRx+'$','i').test(l))units.push(l);});
    refRows.forEach(function(r,idx){
      var cantidad=qtys[idx]||qtys[0]||'', unit=units[idx]||units[0]||'M', ubic=locs[idx]||'';
      if(r.referencia && r.descripcion && cantidad && unit)rows.push({referencia:r.referencia,descripcion:r.descripcion,cantidad:cantidad,unidad:unit,ubicacion:ubic,pagina:1,mode:'V205_COLUMN_TEXT'});
    });
  }
  return eiV92DedupeItems(rows);
}
var eiV205LegacyParseRows = eiV92ParseSiesaRows;
eiV92ParseSiesaRows = function(words,pageNum,pageWidth,pageHeight){
  ...
  for(var li=0;li<lines.length;li++){
    ...
    desc=eiV205CleanDescTail(desc).replace(/\bPARQUE\s+INDUSTRIAL\b/ig,' ').replace(/\s+/g,' ').trim();
    var ubic=eiV92LineTextInRange(line,ubicMin,ubicMax);
    if(!desc)return;
    rows.push({referencia:ref,descripcion:desc,cantidad:qty,unidad:unit,ubicacion:ubic,pagina:pageNum,mode:'V205_GEOMETRY'});
  }
  return rows;
};
var eiV205ReadPdfFileBase = readPdfFile;
readPdfFile = function(file){
  return eiV205ReadPdfFileBase(file).then(function(text){
    var rawRows=eiV205ParseSiesaRowsFromRawText(text);
    if(!rawRows.length)return text;
    var existing=[]; try{existing=extractPedidoItems(text)||[];}catch(e){existing=[];}
    var merged=eiV92DedupeItems((existing||[]).concat(rawRows.map(function(m,i){return eiV92MaterialToApp(m,i,'V205_RAW_TEXT');})));
    ...
    var markers=merged.map(function(it,idx){return '__EI_V92_ROW__'+JSON.stringify(eiV92MaterialToApp(it,idx,'V205_MERGE'));}).join('\n');
    var audit='__EI_V92_AUDIT__'+JSON.stringify({version:'V205',materialesExtraidos:merged.length,rawExtraidos:rawRows.length,nota:'Incluye soporte para cantidades con miles 20.000,00 y filas SIESA multilinea.'});
    return markers+'\n'+audit+'\n'+clean;
  });
};


/* V205 QA ERP final: refuerzo conservador sin cambiar flujo.
...
function eiV205OperationalHealthCheck(){
  ...
  if(missing.length){console.error("QA ERP V205 · funciones críticas faltantes:",missing);}
  else{console.info("QA ERP V205 · funciones críticas disponibles.");}
  return missing;
}
var eiV205LegacyOpenCutsPlanner = openCutsPlanner;
openCutsPlanner = function(id){
  ...
  qs("#cutsPlanForm").onsubmit=function(e){
    ...
    applyAlistamientoAutoChecklist(c);
    persistCase(c,{type:"CUT_REQUESTS_SYNCED_V205",detail:"Corte/Alistamiento sincronizado. Cortes nuevos: "+added+". Total antes: "+before+", total ahora: "+(c.cutRequests||[]).length}).then(function(){closeDrawer();renderDetail(id);}).catch(function(e){showError(e.message||e);});
  };
};
try{eiV205OperationalHealthCheck();}catch(e){console.warn("QA ERP V205 no pudo ejecutarse",e);}
...
/* V205 - tamaño popup Recepción de pedidos PC */
try{
