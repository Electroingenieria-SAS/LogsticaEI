...
var EI_CANONICAL_APP_VERSION = "v210-corte-movil-simple-sin-cronometro";
try{
...
function ensureV210UiFixes(){
  if(document.getElementById("ei-v210-corte-movil-simple-sin-cronometro"))return;
  var st=document.createElement("style");
  st.id="ei-v210-corte-movil-simple-sin-cronometro";
  st.textContent='\
\
\
\
...
function forceCajaPvcPveToCarteraNow(){
  ...
  (state.cases||[]).forEach(function(c){
    if(migrateCajaToCarteraIfNeeded(c,"Normalización masiva V210: PVC/PVE debe estar en Cartera.")){
      count++;
...
function autoNormalizeLoadedPvcPveFromCajaToCartera(reason){
  ...
  (state.cases||[]).forEach(function(c){
    ...
    if(migrateCajaToCarteraIfNeeded(c,reason||"Normalización automática V210: PVC/PVE no puede permanecer en Caja.")){
      count++;
      ...
      jobs.push(db.collection("cases").doc(c.id).set(c,{merge:true}).then(function(){
        return createEvent({type:"AUTO_CARTERA_REDIRECT_V210",caseId:c.id,process:"cartera",detail:"PVC/PVE detectado en Caja y redirigido automáticamente a Cartera.",targetRole:"cartera",visibleRoles:financialVisibleRolesForOrder(c)}).catch(function(){return null;});
...
function ensureMobileFreshVersion(){
  try{
    var version="v210-corte-movil-simple-sin-cronometro";
...
function loadData(){
  ...
  return Promise.all([
    ...
    state.dataLoading=false;
    var normalizePromise=autoNormalizeLoadedPvcPveFromCajaToCartera("Normalización al cargar datos V210: PVC/PVE debe ir a Cartera.");
    ...
    return normalizePromise.then(function(moved){
      if(moved>0)console.info("[V210] PVC/PVE movidos automáticamente de Caja a Cartera:",moved);
    });
  }).catch(function(e){
    state.dataLoading=false;
    throw e;
...
function autoMigrateLegacyProcesses(){
  ...
  state.cases.forEach(function(c){
    ...
    if((currentUserIsAdminOrSuper() || normalizeRole(state.user&&state.user.role)==="gerencia") && forcePveCaseToPurchasesInMemory(c,"Migración automática V210: PVE existente debe pasar por Compras antes de Recepción"))changed=true;
...
function forceExistingPveToPurchasesNow(){
  ...
  db.collection("cases").get().then(function(snap){
    ...
    list.forEach(function(c){
      ...
      if(forcePveCaseToPurchasesInMemory(c,"Migración manual V210: PVE existente enviado a Compras antes de Recepción")){
...
function loadWarningsHtml(){
  ...
  return '<div class="alert warning"><strong>Algunos módulos no cargaron por reglas/permisos pendientes.</strong><br>'+esc(state.loadWarnings.slice(0,3).join(" | "))+'<br><small>La app no se detiene; publique las reglas V210 y recargue la PWA.</small></div>';
}
...
function ensureLowResolutionResponsiveCss(){
  if(document.getElementById("ei-v210-corte-movil-simple-sin-cronometro")){applyLowResolutionMode();return;}
  var st=document.createElement("style");
  st.id="ei-v210-corte-movil-simple-sin-cronometro";
  st.textContent='\
...
function ensureV210ViewportFinalFix(){
  try{
...
@media(max-width:790px){
  ...
  }catch(e){console.warn("No se pudo aplicar V210 viewport fix",e);}
}
...
function temporaryProfileFromAuth(fbUser,reason){
  ...
  state.route=defaultRoute(state.user.role);
  state.loadWarnings=state.loadWarnings||[];
  state.loadWarnings.push("Perfil temporal: Firebase no permitió leer users/"+((fbUser&&fbUser.uid)||"")+". La app abrió para no parar el proceso. Publique reglas V210 y cree/verifique el perfil operativo.");
  sessionStorage.setItem(storageKey+"_session",JSON.stringify(state.user));
...
function createCase(fd){
  ...
  ensurePvePurchasingMetadata(c,"Creación V210: pedido PVE debe pasar por Compras antes de Recepción");
  var finish=function(){
...
function renderDetail(id){
  ...
  if(migrateCajaToCarteraIfNeeded(c,"Corrección automática V210: PVC/PVE debe ir a Cartera, no Caja."))correctedOnOpen=true;
  if(caseLooksBlockedInSales(c)){
    if(releaseAnyCaseIfResolvedByReqOrReport(c,"Corrección automática V210: requerimiento o novedad/reporte ya respondido/cerrado.","Corrección automática V210"))correctedOnOpen=true;
  }
  if(migrateLegacyCaseInMemory(c,"Corrección automática al abrir detalle"))correctedOnOpen=true;
  if((currentUserIsAdminOrSuper() || normalizeRole(state.user&&state.user.role)==="gerencia") && forcePveCaseToPurchasesInMemory(c,"Corrección automática V210 al abrir detalle: PVE debe pasar por Compras"))correctedOnOpen=true;
  ...
  if(!c.closedAt){
    ...
    if(c.status==="en_proceso"&&c.currentProcess==="recepcion_pedidos"&&canOperate)actions+='<button class="btn btn-primary" data-action="receptionPdf" data-id="'+c.id+'">Cargar PDF o CSV recepción · V210</button>';
...
function casePdfLoadedAt(c){
  return c && eiV210IsReceptionSupportLoaded(c) ? c.documentFlow.receptionFileLoadedAt : "";
}
...
function mergePdfItemsIntoCase(c, parsed){
  ...
  // V210: la clave de una línea de recepción incluye ubicación/bodega/extensiones.
...
function repairReceptionLinesFromPdfExtraction(c){
  ...
  if(added>0 || after!==before){
    ...
    c.documentFlow.receptionLinesRepairDetail="Corrección automática V210: se restauraron líneas iguales con ubicaciones distintas para que todos los carretos completos pasen a alistamiento.";
...
/* ============================================================
   V210 - Recepción de pedidos: dos botones separados PDF / CSV
   Alcance único:
...
function eiV210CsvRows(text){
  ...
  for(var r=start+1;r<lines.length;r++){
    var cols=eiV210SplitCsvLine(lines[r],delim);
    ...
    var it={
      id:'CSV-V210-'+String(rows.length+1).padStart(4,'0'),
      referencia:String(ref).replace(/\D/g,''),
      descripcion:String(desc||'').trim(),
      cantidad:eiV210NormalizeCsvQty(qty),
      ...
      generatedBy:'CSV_RECEPCION_PEDIDOS_V210',
      origen:'CSV_RECEPCION_PEDIDOS_V210',
      detectionReason:'Fila cargada desde CSV V210 en Recepción de pedidos.'
    };
...
function eiV210ReadTextFile(file){
  return new Promise(function(resolve,reject){
    var reader=new FileReader();
...
function eiV210ParsedFromCsv(text,c,fileName){
  var items=eiV210CsvRows(text);
...
function eiV210ApplyCsvItemsToCase(c,parsed){
  ...
  c.orderItems=c.orderItems.filter(function(it){
    ...
    return origin.indexOf('CSV_RECEPCION_PEDIDOS_V210')<0;
  });
...
/* V210 - Helper seguro para abrir/cerrar drawer de Recepción de pedidos */
function eiV210CloseReceptionDrawer(){
  try{
    var d=qs("#drawer");
    if(d){
...
function eiV210OpenDrawerHtml(html){
  var d=qs("#drawer");
  ...
  d.onclick=function(ev){
    ...
    if(t===d || (t && t.closest && t.closest('[data-action="closeDrawer"],[data-action="closeReceptionDrawer"]'))){
      ev.preventDefault();
      ev.stopPropagation();
      eiV210CloseReceptionDrawer();
    }
  };
  try{
    document.onkeydown=function(ev){
      if(ev && ev.key==="Escape" && d.classList.contains("open"))eiV210CloseReceptionDrawer();
    };
  }catch(e){}
}
function eiV210NormalizeCsvQty(v){
  var s=String(v||'').trim();
  if(!s)return '';
  s=s.replace(/\s+/g,'');
...
/* V210 - Helper local para guardar datos del pedido sin depender de applyParsedPedidoToCase */
function eiV210ApplyParsedPedidoToCase(c,parsed){
  parsed=parsed||{};
  var changed=[];
...
function eiV210EnsureReceptionDecision(c,parsed){
...
/* V210 - PDF y CSV son soporte válido para Recepción de pedidos */
function eiV210IsReceptionSupportLoaded(c){
  c=c||{};
...
function eiV210MarkReceptionSupport(c,up,mode,fileName,parsed){
  c.documentFlow=c.documentFlow||{};
...
function openReceptionPdf(id){
  ...
  drawer('<section class="modal modal-wide ei207-reception-modal" style="width:min(1180px,96vw);max-width:96vw;max-height:92vh;overflow:auto;border-radius:24px"><div class="modal-head"><div><h3>Recepción de pedidos · cargar PDF o CSV · V210</h3><p>PDF y CSV usan el mismo ciclo: compromiso, decisión de corte, líneas, cortes automáticos, checklist, evidencia y evento.</p></div><button class="btn btn-small" type="button" data-action="closeDrawer">Cerrar</button></div>'+
    '<form class="form" id="recPdfForm">'+
    '<div class="notice success"><strong>V210:</strong> escoja una carga. El CSV conserva todas las filas físicas y al guardar sigue el mismo ciclo operativo del PDF.</div>'+
    '<div class="grid2">'+
  ...
  if(csvInput){
    csvInput.onchange=function(e){
      ...
      qs("#pdfExtractPreview").innerHTML="";
      eiV210ReadTextFile(f).then(function(text){
        parsed=eiV210ParsedFromCsv(text,c,f.name);
        parsed.extractionMode="CSV_RECEPCION_PEDIDOS_V210";
  ...
  qs("#recPdfForm").onsubmit=function(e){
    ...
    parsed.items.forEach(function(it,idx){
      if(parsed.extractionMode==="CSV_RECEPCION_PEDIDOS_V210"){
        it.origen="CSV_RECEPCION_PEDIDOS_V210";
        it.generatedBy="CSV_RECEPCION_PEDIDOS_V210";
    ...
    if(parsed.extractionMode==="CSV_RECEPCION_PEDIDOS_V210"){
      c.documentFlow.extractionMode="CSV_RECEPCION_PEDIDOS_V210";
    }
    ...
    uploadReceptionPdfToDrive(selectedFile,c).then(function(up){
      eiV210MarkReceptionSupport(c,up,loadedMode||parsed.extractionMode,fileName,parsed);
...
function ensureCutCaseFlagsV210(c){
  try{
    if(!c)return;
    c.hasCuts=true;
...
function renderCutsQueue(){
  ...
  if(isMobileRuntime()||isCutRuntimeUser()){
    ...
    layout(
      ...
      '<section class="ei191-cut-kpis"><article><span>Pendientes</span><strong>'+rows.length+'</strong></article><article><span>Grupos</span><strong>'+groupKeys.length+'</strong></article><article><span>Versión</span><strong>V210</strong></article></section>'+
...
function forceReleaseOneSalesBlock(id){
  ...
  if(!releaseCaseFromSalesBlock(c,"Liberación manual V210 por rol autorizado.","Liberación manual V210")){alert("No fue posible liberar este pedido.");return;}
  persistCase(c,{type:"SALES_BLOCK_MANUAL_RELEASED",detail:"Liberación manual V210: pedido devuelto a "+processTitle(c.currentProcess),targetRole:c.assignedRole,visibleRoles:["admin","super_admin","super_administrador","gerencia","jefe_logistica",c.assignedRole,"ventas"]}).then(loadData).then(function(){renderDetail(id);alert("Pedido liberado de Ventas y devuelto a "+processTitle(c.currentProcess)+".");}).catch(function(e){showError((e&&e.message)||e||"No se pudo liberar el pedido.");});
}
...
function forceReleaseResolvedSalesBlocksNow(){
  ...
  (state.cases||[]).forEach(function(c){
    ...
    if(releaseCaseFromSalesBlock(c,"Reparación V210: requerimiento o novedad/reporte respondido/cerrado. Incluye liberación aunque exista requerimiento y reporte al mismo tiempo.","Reparación V210")){
      changed.push(c);
      promises.push(persistCase(c,{type:"SALES_BLOCK_FORCE_RELEASED_V210",detail:"Reparación V210: pedido liberado de Ventas y devuelto a "+processTitle(c.currentProcess),targetRole:c.assignedRole,visibleRoles:["admin","super_admin","super_administrador","gerencia","jefe_logistica",c.assignedRole,"ventas"]}));
    }
  });
...
function ensureV210CutUiFixes(){
  if(document.getElementById('ei-v210-corte-movil-simple-sin-cronometro'))return;
  var st=document.createElement('style');st.id='ei-v210-corte-movil-simple-sin-cronometro';
  ...
  (state.reports||[]).forEach(function(r){
    ...
    if(cleaned.changed || wrongMerge){
      ...
      var payload={
        ...
        lastUpdateType:"CORRECCION_FORZADA_TRAZABILIDAD_V210",
        updatedAt:stamp
      };
      if(wrongMerge){
...
function downloadKpiExcel(){
  ...
  var head='<html><head><meta charset="utf-8"><style>body{font-family:Century Gothic,Arial}h1{color:#061B46}.kpi{font-size:18px;font-weight:bold;color:#061B46}table{border-collapse:collapse;width:100%;margin-bottom:18px}th{background:#061B46;color:white}td,th{border:1px solid #cbd5e1;padding:8px;vertical-align:top}.note{background:#f8fafc;border:1px solid #cbd5e1;padding:10px}.warn{background:#fff7ed}.total-hours{font-weight:bold;background:#eaf2ff}</style></head><body>'+
    ...
    '<div class="note"><strong>V210:</strong> exportación por lotes para evitar que el navegador se quede sin responder. La primera tabla sigue siendo la base del informe: DEMORA EXACTA POR PEDIDO.</div>';
  parts.push(head);
...
function renderIndicators(){
  ...
  layout(header("VSM ERP · Normal / PVE","Panel liviano con VSM normal y VSM PVE separados, más normalización de horas por pedido/día.",'<a class="btn btn-primary" href="'+link+'" target="_blank" rel="opener">Abrir VSM optimizado</a>'+(canExportSiesa?'<button class="btn btn-gold" data-action="exportSiesaCuts">Exportar plano SIESA cortes</button>':''))+
    ...
    '<section class="grid grid-3" style="margin-top:16px">'+
      ...
      '<article class="card kpi"><span>Exportación</span><strong style="font-size:1.25rem">Excel V210</strong><small>Tablas separadas para informe completo.</small></article>'+ 
    '</section>'+ 
...
function assignToProcess(c,next,detail,assignmentUsers){
  if(c && next==="recepcion_pedidos" && pveShouldBeForcedToPurchases(c)){
    ensurePvePurchasingMetadata(c,"Bloqueo V210: ningún PVE sin liberar por Compras puede entrar directo a Recepción");
    next="compras";
...
function renderCutDiagnostics(){
  ...
  layout(header("Diagnóstico Corte","Verificación rápida de perfil, permisos y carga para el usuario de Corte.",'<button class="btn btn-gold" data-action="forceRefreshCases">Probar carga de pedidos</button>')+
    ...
    '<section class="notice"><strong>Uso:</strong> si aquí el rol no sale como auxiliar_corte o no cargan casos, debe publicarse firestore.rules V210 y verificar el documento users del UID real.</section>');
}
function render(){
...
/* V210: lectura reforzada de PDF SIESA con cantidades 20.000,00 y filas multilinea; no elimina lector V92. */
function eiV210QtyRegex(){return /^\d{1,3}(?:[.,]\d{3})*(?:[.,]\d+)?$|^\d+(?:[.,]\d+)?$/;}
function eiV210CleanDescTail(s){return eiV92CleanText(String(s||'').replace(/\$\s*[\d.,]+.*$/g,'').replace(/\b(?:VALOR|UNIT|PARCIAL|SUBTOTAL|IVA|TOTAL)\b.*$/ig,''));}
function eiV210ParseSiesaRowsFromRawText(raw){
  ...
  while((m=rx.exec(text))){
    ...
    desc=eiV210CleanDescTail(desc).replace(/\s+/g,' ').trim();
    desc=desc.replace(/\bPARQUE\s+INDUSTRIAL\b/ig,' ').replace(/\s+/g,' ').trim();
    if(eiV92ReferenceOk(reference) && desc && cantidad && ubic){rows.push({referencia:reference,descripcion:desc,cantidad:cantidad,unidad:unit,ubicacion:ubic,pagina:1,mode:'V210_RAW'});}
  }
  ...
  if(!rows.length){
    ...
    var locs=[], qtys=[], units=[];
    lines.forEach(function(l){if(/^[A-Z]\d{5,6}$/.test(l))locs.push(l); if(eiV210QtyRegex().test(l))qtys.push(l); if(new RegExp('^'+unitRx+'$','i').test(l))units.push(l);});
    refRows.forEach(function(r,idx){
      var cantidad=qtys[idx]||qtys[0]||'', unit=units[idx]||units[0]||'M', ubic=locs[idx]||'';
      if(r.referencia && r.descripcion && cantidad && unit)rows.push({referencia:r.referencia,descripcion:r.descripcion,cantidad:cantidad,unidad:unit,ubicacion:ubic,pagina:1,mode:'V210_COLUMN_TEXT'});
    });
  }
  return eiV92DedupeItems(rows);
}
var eiV210LegacyParseRows = eiV92ParseSiesaRows;
eiV92ParseSiesaRows = function(words,pageNum,pageWidth,pageHeight){
  ...
  for(var li=0;li<lines.length;li++){
    ...
    desc=eiV210CleanDescTail(desc).replace(/\bPARQUE\s+INDUSTRIAL\b/ig,' ').replace(/\s+/g,' ').trim();
    var ubic=eiV92LineTextInRange(line,ubicMin,ubicMax);
    if(!desc)return;
    rows.push({referencia:ref,descripcion:desc,cantidad:qty,unidad:unit,ubicacion:ubic,pagina:pageNum,mode:'V210_GEOMETRY'});
  }
  return rows;
};
var eiV210ReadPdfFileBase = readPdfFile;
readPdfFile = function(file){
  return eiV210ReadPdfFileBase(file).then(function(text){
    var rawRows=eiV210ParseSiesaRowsFromRawText(text);
    if(!rawRows.length)return text;
    var existing=[]; try{existing=extractPedidoItems(text)||[];}catch(e){existing=[];}
    var merged=eiV92DedupeItems((existing||[]).concat(rawRows.map(function(m,i){return eiV92MaterialToApp(m,i,'V210_RAW_TEXT');})));
    ...
    var markers=merged.map(function(it,idx){return '__EI_V92_ROW__'+JSON.stringify(eiV92MaterialToApp(it,idx,'V210_MERGE'));}).join('\n');
    var audit='__EI_V92_AUDIT__'+JSON.stringify({version:'V210',materialesExtraidos:merged.length,rawExtraidos:rawRows.length,nota:'Incluye soporte para cantidades con miles 20.000,00 y filas SIESA multilinea.'});
    return markers+'\n'+audit+'\n'+clean;
  });
};


/* V210 QA ERP final: refuerzo conservador sin cambiar flujo.
...
function eiV210OperationalHealthCheck(){
  ...
  if(missing.length){console.error("QA ERP V210 · funciones críticas faltantes:",missing);}
  else{console.info("QA ERP V210 · funciones críticas disponibles.");}
  return missing;
}
var eiV210LegacyOpenCutsPlanner = openCutsPlanner;
openCutsPlanner = function(id){
  ...
  qs("#cutsPlanForm").onsubmit=function(e){
    ...
    applyAlistamientoAutoChecklist(c);
    persistCase(c,{type:"CUT_REQUESTS_SYNCED_V210",detail:"Corte/Alistamiento sincronizado. Cortes nuevos: "+added+". Total antes: "+before+", total ahora: "+(c.cutRequests||[]).length}).then(function(){closeDrawer();renderDetail(id);}).catch(function(e){showError(e.message||e);});
  };
};
try{eiV210OperationalHealthCheck();}catch(e){console.warn("QA ERP V210 no pudo ejecutarse",e);}
...
/* V210 - tamaño popup Recepción de pedidos PC */
try{
...
/* V210 - Merge físico CSV solo para Recepción de pedidos.
   Mantiene el ciclo original: mergePdfExtractionIntoCase -> autoCreateCutsFromItems -> checklist -> evidencia -> evento.
*/
var eiV210LegacyMergePdfItemsIntoCase = mergePdfItemsIntoCase;
mergePdfItemsIntoCase=function(c,parsed){
  parsed=parsed||{};
  if(parsed.extractionMode!=="CSV_RECEPCION_PEDIDOS_V210"){
    return eiV210LegacyMergePdfItemsIntoCase(c,parsed);
  }
  var incoming=(parsed.items||[]).map(function(it,idx){
    var copy=Object.assign({},it);
    copy.id=uid("LIN");
    copy.estado=copy.requiereCorte ? "PENDIENTE_CORTE" : "PENDIENTE_ALISTAMIENTO";
    copy.origen="CSV_RECEPCION_PEDIDOS_V210";
    copy.generatedBy="CSV_RECEPCION_PEDIDOS_V210";
    copy.createdAt=now();
    copy.csvPhysicalRowNumber=idx+1;
    copy.detectionReason=(copy.detectionReason||"")+" · CSV V210 conservado como fila física independiente.";
    return copy;
  });
  c.orderItems=c.orderItems||[];
  c.orderItems=c.orderItems.filter(function(it){
...
function eiV210IsFabianDuqueRuntimeUser(){
  var raw=eiV210IdentityText(state.user||{});
  return raw.indexOf("f.duque@ei.com.co")>=0 ||
         raw.indexOf("f.duque")>=0 ||
         raw.indexOf("fabian duque")>=0 ||
         raw.indexOf("fabian")>=0;
}
function eiV210IsCutWorkerRuntimeUser(){
  ...
  return r==="auxiliar_corte" ||
         ...
         r==="operario_corte" ||
         r==="operario_de_corte" ||
         eiV210IsFabianDuqueRuntimeUser();
}
function eiV210RepairCutRuntimeUser(){
  try{
    if(!state.user)return;
    if(eiV210IsFabianDuqueRuntimeUser() || normalizeRole(state.user.role)==="auxiliar_corte"){
      state.user.role="auxiliar_corte";
...
function eiV210CanOperateCutModule(){
  eiV210RepairCutRuntimeUser();
  var r=state.user?normalizeRole(state.user.role):"";
  return eiV210IsCutWorkerRuntimeUser() ||
         r==="jefe_logistica" ||
         r==="gerencia" ||
         isAdminRoleValue(r) ||
...
isCutWorkerUser=function(){
  return eiV210CanOperateCutModule();
};
...
function eiV210EnsureMobileCutCase(c,cut){
  try{
    if(!c)return;
    eiV210RepairCutRuntimeUser();
    c.hasCuts=true;
...
var eiV210LegacyOpenCutModule=openCutModule;
openCutModule=function(id,cutId){
  ...
  if(c){
    ...
    eiV210EnsureMobileCutCase(c,cut);
  }
  return eiV210LegacyOpenCutModule(id,cutId);
};
var eiV210LegacyHandleCutAction=handleCutAction;
handleCutAction=function(c,cut,action){
  eiV210RepairCutRuntimeUser();
  eiV210EnsureMobileCutCase(c,cut);
  if(!eiV210CanOperateCutModule()){
    alert("No tiene permiso para operar este corte. Verifique que el usuario esté como Auxiliar de corte.");
    return;
  }
  return eiV210LegacyHandleCutAction(c,cut,action);
};
try{eiV210RepairCutRuntimeUser();}catch(e){}



/* V210_CORTE_MOVIL_SIMPLE_SIN_CRONOMETRO
   Corte móvil simple:
...
*/
function eiV210InjectSimpleMobileCutCss(){
  try{
...
try{eiV210InjectSimpleMobileCutCss();}catch(e){}

function eiV210ApplyMobileCutSimpleView(){
  try{
    if(!(isMobileRuntime && isMobileRuntime()))return;
    eiV210InjectSimpleMobileCutCss();
...
var eiV210LegacyOpenCutModule=openCutModule;
openCutModule=function(id,cutId){
  var result=eiV210LegacyOpenCutModule(id,cutId);
  setTimeout(eiV210ApplyMobileCutSimpleView,0);
  setTimeout(eiV210ApplyMobileCutSimpleView,80);
  return result;
};
