...
function receptionStickerPrintHtml(stickers,title){
  ...
  var css='@page{size:A4;margin:8mm}*{box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;margin:0;color:#0f172a;background:#f8fafc}.no-print{padding:10px 0;margin:0 0 10px 0;text-align:center}.no-print button{border:0;border-radius:12px;background:#082a63;color:#fff;font-weight:800;padding:10px 18px;cursor:pointer;box-shadow:0 8px 20px rgba(8,42,99,.18)}.sheet{display:grid;grid-template-columns:repeat(2,1fr);gap:8mm}.label{min-height:62mm;border:1.5px solid #0f2f66;border-radius:16px;padding:0;break-inside:avoid;page-break-inside:avoid;display:flex;flex-direction:column;overflow:hidden;background:#fff;box-shadow:0 10px 28px rgba(15,23,42,.08)}.top{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 12px;background:linear-gradient(135deg,#082a63 0%,#184a99 100%);color:#fff}.brandbox{display:flex;align-items:center;gap:10px;min-width:0}.logo{width:44px;height:44px;object-fit:contain;background:#fff;border-radius:10px;padding:4px;flex:0 0 auto}.brandline{font-size:10px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;opacity:.92}.title{font-size:16px;font-weight:900;line-height:1.05;margin-top:2px}.state{border-radius:999px;padding:5px 10px;font-size:11px;font-weight:900;background:#dcfce7;color:#166534;border:1px solid rgba(255,255,255,.35);white-space:nowrap}.oc-strip{display:flex;justify-content:space-between;align-items:flex-start;gap:10px;padding:10px 12px 8px 12px;border-bottom:1px solid #dbe4f0;background:#f8fbff}.oc-wrap small,.meta small{display:block;font-size:9px;text-transform:uppercase;letter-spacing:.08em;color:#475569;margin-bottom:2px}.oc-wrap strong{display:block;font-size:16px;color:#082a63;line-height:1.1}.material{padding:10px 12px 2px 12px}.material small{display:block;font-size:9px;text-transform:uppercase;letter-spacing:.08em;color:#64748b;margin-bottom:2px}.material strong{display:block;font-size:15px;color:#0f172a;line-height:1.18}.grid{display:grid;grid-template-columns:1fr 1fr;gap:7px 10px;padding:8px 12px 12px 12px}.cell{background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:7px 8px;min-height:42px}.cell span{display:block;color:#64748b;font-size:9px;text-transform:uppercase;letter-spacing:.08em;margin-bottom:2px}.cell b{display:block;font-size:11px;color:#0f172a;line-height:1.18}.foot{margin-top:auto;display:flex;justify-content:space-between;gap:8px;align-items:flex-end;padding:8px 12px 10px 12px;border-top:1px dashed #cbd5e1;background:#fcfdff}.foot .meta{font-size:9px;color:#475569}.code{font-family:Consolas,Monaco,monospace;font-size:10px;font-weight:700;color:#082a63;background:#eaf1ff;padding:5px 8px;border-radius:999px;border:1px solid #c7d7ff}.note{font-size:9px;color:#64748b;text-align:right}@media print{body{background:#fff}.no-print{display:none}.sheet{gap:6mm}.label{box-shadow:none}.top{print-color-adjust:exact;-webkit-print-color-adjust:exact}.state{print-color-adjust:exact;-webkit-print-color-adjust:exact}}';
  var rows=stickers.map(function(st){
    ...
    return '<article class="label"><div class="top"><div class="brandbox">'+(logoUrl?'<img class="logo" src="'+esc(logoUrl)+'" alt="Logo">':'')+'<div><div class="brandline">Electroingeniería</div><div class="title">Sticker de separación</div></div></div><div class="state">'+esc(st.status||'Conforme')+'</div></div><div class="oc-strip"><div class="oc-wrap"><small>Orden de compra / PVE</small><strong>'+esc(po)+'</strong></div><div class="meta"><small>Documento ingreso</small><strong style="font-size:12px;color:#0f172a">'+esc(st.documentNumber||'')+'</strong></div></div><div class="material"><small>Material</small><strong>'+esc(st.materialName||st.material||'Material sin nombre')+'</strong></div><div class="grid"><div class="cell"><span>Referencia</span><b>'+esc(st.reference||st.referencia||'')+'</b></div><div class="cell"><span>Cantidad</span><b>'+esc(qty)+'</b></div><div class="cell"><span>Ubicación</span><b>'+esc(st.location||st.ubicacion||'')+'</b></div><div class="cell"><span>Medidas</span><b>'+esc(st.measures||st.medidas||'')+'</b></div><div class="cell"><span>Fecha de ingreso</span><b>'+esc(st.entryDate||'')+'</b></div><div class="cell"><span>Responsable</span><b>'+esc(st.responsibleName||st.responsableIngreso||'')+'</b></div><div class="cell"><span>Proveedor</span><b>'+esc(st.supplier||'')+'</b></div><div class="cell"><span>Estado impresión</span><b>'+esc(st.printStatus||'PENDIENTE_IMPRESION')+'</b></div></div><div class="foot"><div class="meta"><small>Generado</small><strong style="font-size:10px;color:#0f172a">'+esc(new Date().toLocaleString('es-CO'))+'</strong></div><div class="note">Pegar en la mercancía<br><span class="code">'+esc(st.id||'')+'</span></div></div></article>';
  }).join('');
