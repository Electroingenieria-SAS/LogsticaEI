...
function renderSummary(){
  ...
  var split='<section class="grid grid-2 vsm-split" style="margin-top:10px">'
    ...
    +'</section>';
  $('summary').innerHTML=[
    ['Tipo VSM',m.vsmType,'Filtro activo.','Normal / PVE'],
    ['Pedidos',m.cases,'Registros incluidos.','Base'],
  ...
  ].map(function(c){return kpiCard(c[0],c[1],c[2],c[3]);}).join('')+split;
  var b=m.bottleneck;
...
function renderTable(){var view=$('fView').value,m=app.metrics;if(!m)return;var html='',title='',count=0;if(view==='procesos'){title='Lead time por proceso';count=m.processRows.length;html=table(['Macroproceso','Casos','WIP','LT prom.','Total','Ocupación','Espera','NVA','% VA','% Espera','% NVA','Req. h','Cortes'],m.processRows.map(function(r){return '<tr><td>'+esc(r.label)+'</td><td>'+r.cases+'</td><td>'+r.wip+'</td><td>'+fmt(r.avg)+'</td><td>'+timeUnit(r.total)+'</td><td>'+timeUnit(r.active)+'</td><td>'+timeUnit(r.wait)+'</td><td>'+timeUnit(r.dead)+'</td><td>'+r.eff+'%</td><td>'+r.waitPct+'%</td><td>'+r.deadPct+'%</td><td>'+timeUnit(r.req)+'</td><td>'+r.doneCuts+'/'+r.cuts+'</td></tr>';}));}else if(view==='usuarios'){title='Productividad por usuario';count=m.userRows.length;html=table(['Usuario','Rol','Casos','Abiertos','Cerrados','LT prom.','Total','Ocupación','Espera','NVA','% VA','% Espera','Productividad','Procesos trazados'],m.userRows.map(function(r){return '<tr><td><strong>'+esc(r.user)+'</strong></td><td>'+esc(roleTitle(r.role))+'</td><td>'+r.count+'</td><td>'+r.open+'</td><td>'+r.closed+'</td><td>'+fmt(r.avg)+'</td><td>'+timeUnit(r.total)+'</td><td>'+timeUnit(r.active)+'</td><td>'+timeUnit(r.wait)+'</td><td>'+timeUnit(r.dead)+'</td><td>'+r.eff+'%</td><td>'+r.waitPct+'%</td><td>'+r.productivity+'</td><td>'+esc(r.processList||'')+'</td></tr>';}));}else if(view==='usuario_proceso'){title='Detalle usuario por proceso';count=m.userProcessRows.length;html=table(['Usuario','Rol','Proceso','Casos','Abiertos','Cerrados','LT prom.','Total','Ocupación','Espera','NVA','% VA','% Espera','Req. h','Cortes'],m.userProcessRows.map(function(r){return '<tr><td>'+esc(r.user)+'</td><td>'+esc(roleTitle(r.role))+'</td><td>'+esc(r.label)+'</td><td>'+r.count+'</td><td>'+r.open+'</td><td>'+r.closed+'</td><td>'+fmt(r.avg)+'</td><td>'+timeUnit(r.total)+'</td><td>'+timeUnit(r.active)+'</td><td>'+timeUnit(r.wait)+'</td><td>'+timeUnit(r.dead)+'</td><td>'+r.eff+'%</td><td>'+r.waitPct+'%</td><td>'+timeUnit(r.req)+'</td><td>'+Math.round(r.cuts)+'</td></tr>';}));}else if(view==='cancelados'){title='Pedidos cancelados / anulados';count=m.cancelRows.length;html=table(['Pedido','OC','Cliente','Asesor','Tipo','Proceso donde se canceló','Fecha cancelación','Usuario','Motivo','PDF soporte'],m.cancelRows.map(function(r){return '<tr><td><strong>'+esc(r.pedido)+'</strong></td><td>'+esc(r.oc)+'</td><td>'+esc(r.cliente)+'</td><td>'+esc(r.asesor)+'</td><td><span class="pill">'+esc(r.tipo)+'</span></td><td>'+esc(r.procesoTxt)+'</td><td>'+esc(dateTxt(r.fecha))+'</td><td>'+esc(r.usuario||'')+'</td><td>'+esc(r.motivo||'')+'</td><td>'+(r.soporte?'<a href="'+esc(r.soporte)+'" target="_blank" rel="noopener">Abrir PDF</a>':'Sin soporte trazado')+'</td></tr>';}));}else if(view==='novedades'){title='Tiempo de respuesta de novedades y reportes';count=m.reportRows.length;html=table(['Pedido','Cliente','Novedad / reporte','Referencia','Estado','Criticidad','Actualizaciones','Primera respuesta','Tiempo respuesta','Cierre','Etiqueta VSM'],m.reportRows.slice(0,700).map(function(r){return '<tr><td><strong>'+esc(r.pedido||'')+'</strong></td><td>'+esc(r.cliente||'')+'</td><td>'+esc(r.title||'')+'</td><td>'+esc(r.reference||'')+'</td><td><span class="pill">'+esc(r.status||'')+'</span></td><td>'+esc(r.severity||'')+'</td><td>'+r.updates+'</td><td>'+(r.pending?'Pendiente':dateTxt(r.firstResponse))+'</td><td><strong>'+timeUnit(r.responseMs)+'</strong></td><td>'+(r.closeMs?timeUnit(r.closeMs):'—')+'</td><td>'+(r.pending?'Sin respuesta trazada':'Respondida')+'</td></tr>';}));}else{title=view==='pedidos'?'Demora exacta por pedido':'Principal VSM';var rows=m.caseRows.slice(0,500);count=m.caseRows.length;html=table(['Tipo','Pedido','OC','Cliente','Proceso','Estado','LT hábil/día','LT total','Días','Lectura de tiempo','Resp. novedades','% Ocupación','Cuello','QA'],rows.map(function(r){var c=r.c;return '<tr><td><span class="pill">'+esc(r.orderType)+'</span></td><td><strong>'+esc(refOf(c))+'</strong></td><td>'+esc(purchase(c))+'</td><td>'+esc(c.client||'')+'</td><td>'+esc(processTitle(c.currentProcess))+'</td><td><span class="pill">'+esc(c.status||'')+'</span></td><td><strong>'+timeUnit(r.leadPerDay)+'</strong><br><small class="muted">por día</small></td><td>'+timeUnit(r.lead)+'<br><small class="muted">'+fmt(r.lead)+'</small></td><td>'+r.orderDays+'</td><td>'+timeSplitHtml(r.va,r.wait,r.dead)+'</td><td>'+(r.reportRows.length?'<strong>'+timeUnit(r.reportResponse)+'</strong><br><small class="muted">'+r.reportRows.length+' novedad(es) · '+r.reportPending+' pend.</small>':'—')+'</td><td>'+pct(r.va,r.lead)+'%</td><td>'+esc(r.bottleneck.label||'')+'</td><td>'+(r.missingStart?'Revisar fecha':'OK')+'</td></tr>';}));}$('tableTitle').textContent=title;$('rowCount').textContent=count+' fila(s) · tabla de pedidos limitada a 500 para mantener fluidez';$('mainTable').innerHTML=html;}
...
function filterCases(){var from=$('fFrom').value,to=$('fTo').value,proc=$('fProcess').value,stat=$('fStatus').value,orderType=($('fOrderType')&&$('fOrderType').value)||'normal',q=lower($('fSearch').value),user=$('fUser').value;if(stat==='cancelled')return [];return app.cases.filter(function(c){if(c.excludeFromKpi||c.excludeFromVsm||isCancelledVsm(c))return false;if(orderType==='pve'&&!isPveCase(c))return false;if(orderType==='normal'&&isPveCase(c))return false;var day=isoDay(caseStartMs(c)||c.updatedAt);if(from&&day&&day<from)return false;if(to&&day&&day>to)return false;if(proc){var has=(c.currentProcess===proc)||(c.processStats&&c.processStats[proc])||allTraceEvents(c).some(function(e){return e.process===proc;})||((proc==='corte_cable')&&(c.cutRequests||[]).length);if(!has)return false;}if(stat==='open'&&isClosed(c))return false;if(stat==='closed'&&!isClosed(c))return false;if(stat==='wait'&&!((c.requirements||[]).length||c.openRequirement||c.waitStartedAt||c.salesHold||c.separationRequest))return false;var txt=lower([refOf(c),idOf(c),purchase(c),c.client,advisor(c),c.assignedName,c.status,processTitle(c.currentProcess)].join(' '));if(q&&txt.indexOf(q)<0)return false;if(user){var hit=processStatsList(c).some(function(pr){return personsForProcess(c,pr).some(function(person){return person.key===user;});});if(!hit)return false;}return true;});}
function filterCancelledCases(){var from=$('fFrom').value,to=$('fTo').value,proc=$('fProcess').value,stat=$('fStatus').value,orderType=($('fOrderType')&&$('fOrderType').value)||'normal',q=lower($('fSearch').value),user=$('fUser').value;if(stat&&stat!=='cancelled')return [];return app.cases.filter(function(c){if(!isCancelledVsm(c))return false;if(orderType==='pve'&&!isPveCase(c))return false;if(orderType==='normal'&&isPveCase(c))return false;var day=isoDay(cancellationDateMs(c)||caseStartMs(c)||c.updatedAt);if(from&&day&&day<from)return false;if(to&&day&&day>to)return false;if(proc){var cp=cancellationProcessKey(c);var has=(cp===proc)||(c.currentProcess===proc)||(c.processStats&&c.processStats[proc])||allTraceEvents(c).some(function(e){return e.process===proc;})||((proc==='corte_cable')&&(c.cutRequests||[]).length);if(!has)return false;}var txt=lower([refOf(c),idOf(c),purchase(c),c.client,advisor(c),c.assignedName,c.status,c.cancelStatusLabel,c.cancellationTypeLabel,c.cancellationReason,c.cancellationDetail,processTitle(c.currentProcess),processTitle(cancellationProcessKey(c))].join(' '));if(q&&txt.indexOf(q)<0)return false;if(user){var hit=processStatsList(c).some(function(pr){return personsForProcess(c,pr).some(function(person){return person.key===user;});});if(!hit)return false;}return true;});}
...
async function loadCases(all){if(!app.db)return;loading(true,'Leyendo Firebase sin bloquear...');var limit=Number($('fLimit').value||600);var batch=all?Math.max(500,limit):limit;var out=all?app.cases.slice():[],seen={};out.forEach(function(c){seen[idOf(c)]=1;});await loadEvents(all?3500:1600);await loadReports(all?1600:1000);
  ...
  app.cases=out;app.loadedAll=!!all;fillFilters();status('Datos reales cargados desde Firebase: '+out.length+' pedido(s) y '+app.events.length+' evento(s) y '+(app.reports||[]).length+' novedad(es). '+VERSION+' calcula Lead Time desde fechas reales, trazas, eventos, procesos, esperas, requerimientos y cortes.','ok');await refresh();}
...
async function exportExcel(){if(!app.metrics)await refresh();var m=app.metrics,parts=[];loading(true,'Preparando Excel VSM completo...');parts.push('<html><head><meta charset="utf-8"><style>body{font-family:Century Gothic,Arial}table{border-collapse:collapse;margin-bottom:24px}th,td{border:1px solid #cbd5e1;padding:6px;font-size:12px}th{background:#061b46;color:#fff}.n{mso-number-format:"0.00"}</style></head><body>');parts.push('<h1>Dashboard VSM ERP · '+VERSION+' · Normalizado por día y separado Normal/PVE</h1><p>Exportado: '+xls(new Date().toLocaleString('es-CO'))+'</p><h2>Fórmulas y criterios</h2><table><tr><th>Indicador</th><th>Fórmula / lectura</th></tr>'+row(['Lead Time pedido','Fin real o corte del análisis - inicio real del pedido. Si faltan campos, se respalda con trazas/eventos y updatedAt.'])+row(['Lead Time normalizado por día','Lead Time acumulado del pedido dividido entre los días calendario que abarca el pedido. Sirve para leer horas reales por pedido/día.'])+row(['VSM normal / PVE','Los PVE se separan porque pasan por Compras y tienen una carga distinta; los demás pedidos se miden como flujo normal.'])+row(['Lead Time proceso','Tiempo transcurrido en cada macroproceso desde processStats + trazas + eventos + estado actual.'])+row(['VA','Tiempo activo registrado o inferido por eventos de trabajo / conformidad / cierre.'])+row(['Espera','Tiempos de espera por proceso + bloqueos + requerimientos + pago/separación.'])+row(['Requerimientos','Desde creación del requerimiento hasta respuesta/cierre o corte.'])+row(['NVA / tiempo muerto','Lead Time - VA - Espera, sin negativos.'])+row(['Eficiencia','VA / Lead Time.'])+row(['Cancelados/anulados','No se incluyen en Lead Time ni productividad. Se reportan en control independiente por tipo, proceso, asesor y soporte.'])+'</table>');
  ...
  var blob=new Blob(['\ufeff'].concat(parts),{type:'application/vnd.ms-excel;charset=utf-8'});var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='VSM_ERP_Normalizado_PVE_v156_'+new Date().toISOString().slice(0,10)+'.xls';document.body.appendChild(a);a.click();setTimeout(function(){URL.revokeObjectURL(a.href);a.remove();},1000);loading(false);status('Excel VSM '+VERSION+' generado correctamente con '+m.cases+' pedido(s).','ok');}
